...
function loadBooksFromLocalStorage(): void {
    ...
    books.forEach((bookData: { title: string; author: string; pages: number; suggestedBy: string; status: string; }) => {
        const book = new Book(bookData.title, bookData.author, bookData.pages, BookFormat.Print, bookData.suggestedBy);
        addBookToList(book, bookData.status);
    });
}
